...
<script>
    ...
    function calculateLineTotal(value,id,price){
        let index = lineTotal.findIndex(function (item){
            return item.id == id;
        })
        // console.log(index);
        if(index != -1){
            lineTotal.splice(index,1);
            lineTotal = [{id:id,quantity:value,price:price,line_total:value*price},...lineTotal];
        }
        else{
            lineTotal = [{id:id,quantity:value,price:price,line_total:value*price},...lineTotal];
        }
    ...
    $(document).ready(function(){
        ...
        async function renderList(list){
            ...
            await products.forEach(function (item){
                ...
                let lineIndex = lineTotal.findIndex(function (data){
                ...
                $('#myTable > tbody').append(`<tr>
                    <td>
                        <input type="text" class="form-control" name="material_name[]" readonly value=`+item.name+`>
                        <input type="hidden" class="form-control material_id" name="material_id[]" value=`+item.id+`>
                    </td>
                    <td>
                        <input type="hidden" class="form-control" name="color_id[]" value=`+item.color_id+`>
                        <input type="text" class="form-control" name="color[]" readonly value=`+item.color_name+`>
                    </td>
                    <td>
                        <input type="text" class="form-control" name="price[]" value=`+item.selling_price+`>
                    </td>
                    <td>
                        <input type="text" class="form-control quantity" id="quantity" value="${lineTotal[lineIndex]?.quantity ? lineTotal[lineIndex]?.quantity : ''}" name="quantity[]" onkeyup="calculateLineTotal(value,${item.id},${item.selling_price})">
                    </td>
                    <td>
                        <input type="text" class="form-control" value="" name="line_total[]" readonly>
                    </td>
                </tr>`);
            })
        }
        ...
        $( "#product_color_id" ).change(function() {
            ...
            $.ajax({
                url: '/admin/material/search/color/'+product_id_number+'/'+product_color_id,
                type: 'GET',
                cache: false,
                datatype: 'application/json',
