    ...
    <div class="card-body">
        <form action="{{ route("admin.expenses.update", [$expense->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('department_id') ? 'has-error' : '' }}">
                <label for="expense_department">Department Name</label>
                <select name="department_id" id="expense_department" class="form-control select2">
                    @foreach($departments as $id => $department)
                        <option value="{{ $id }}" {{ (isset($expense) && $expense->department ? $expense->department->id : old('department_id')) == $id ? 'selected' : '' }}>{{ $department }}</option>
                    @endforeach
                </select>
